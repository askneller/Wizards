...
//            logger.info("smf pos {} {} {}", smallfireball.getX(), smallfireball.getY(), smallfireball.getZ());
        ...
        } else if (event.getSpell() == 2 && event.getBlockPos() != null && !event.getBlockPos().equals(BlockPos.ZERO)) {
            Zombie zombie = new Zombie(EntityType.ZOMBIE, player.level());
            zombie.setPos(event.getBlockPos().getX(), event.getBlockPos().getY(), event.getBlockPos().getZ());
